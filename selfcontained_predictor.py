...
def clean_data(employees,surveys,performance):
    #Employees Table

    #Deal with null values
    employees.EmploymentEndReason = employees.EmploymentEndReason.apply(lambda x: 0 if str(x) == 'nan' else 1)
    ...
    employees.drop('EmploymentStartDate', axis=1,inplace=True)
    employees.drop('EmploymentEndDate',axis=1,inplace=True)
    ...
    labels = emp.EmploymentEndReason
    emp.drop('EmploymentEndReason', axis=1,inplace=True)
    return emp, labels
...
def calculate_corr_(X):
    corr = X.corr()
    colsbefore = set(X.columns)
    ...
    colsafter = set(X.columns)

    print('# of Columns after removal: ', len(list(X.columns)))
...
if __name__ == '__main__':
    data, labels = clean_data(employees,surveys,performance)
    data = calculate_vif_(data)
    data = calculate_corr_(data)
    predict(data,labels)
